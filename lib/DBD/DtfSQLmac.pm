...
{ # BEGIN PACKAGE
	...
	sub data_sources {
    	my $curdir = `pwd`; # full path to the current working directory, followed by a newline
		chomp $curdir;		# my or may not end with a colon ':', see below
		
		# there is a trailing ':', if the full path is a volume name.
		# if the full path is a directory, there is no colon at the end (true for MacPerl 5.004).		
			
		$curdir =~ s/:$// ; # get rid of the ':', if there's a trailing colon				
...
{ # BEGIN PACKAGE
    ...
    sub bind_param {
		...
		my $type = (ref $attr) ? $attr->{TYPE} : $attr; # $type is a DBI SQL type number or undef if 
														# $attr not specified		
		$type = ($type) ? _map2dtftype($type) : undef;	# map the DBI SQL type numbers to DtfSQLmac range
		...
		if ( $param_types->[$pNum - 1]) { 	# the type attr has previously been set		
			...
			if ($type) { # ... and valid type hint
				# ... store type hint AND value (if not blob)
				$type_grp = _number_or_string($type); 	# returns num or str or blob type
				if ($type_grp == $DBD::DtfSQLmac::st::_blob) { # blob 
					my $errstr = "ERROR(bind_param): The DtfSQLmac driver doesn\'t support binary data (blob)";
					return $sth->DBI::set_err(64, $errstr); # in fact, returns undef # 64 = Mac::DtfSQL::DTF_ERR_USER()
				} else {
					$param_types->[$pNum - 1] = $type; # supported DBI SQL type number
					$params->[$pNum - 1] = $value;
				}#if
				
			} else { # ... and type hint undef, autsch
	...
	sub execute {
		...
		if (scalar(@params_array) != $numParam) {  # autsch
		...
		if ($numParam > 0) { # if placeholders
			...
			# first, warn if the number of type hints is not what we expected, i.e.
			# placeholders != type hints
				...
				if ($stmt_array[$i] =~ /\?/) { # substr contains '?' placeholder
					...
					for ($j = 0; $j < ($qmark_elements - $sub_last); $j++) { # replace '?' placeholder with actual value
						...
						if (! defined($param_val) ) { # undef = NULL
							...
							# $type_hint may be undef, if number of placeholders != number of type hints 
							$type_hint = @param_types_ary ? shift @param_types_ary : undef; # DBI SQL type number

							# returns num or str, call only if $type_hint not undef
							$type_grp = $type_hint ? _number_or_string($type_hint) : $DBD::DtfSQLmac::st::_other; 	

							if ($type_grp == $DBD::DtfSQLmac::st::_num) { # bind as number
								$qmark_array[$j] = $qmark_array[$j] . $param_val;
							} else { # bind as string, even if type hint is undef
								$qmark_array[$j] = $qmark_array[$j] . "'". $param_val . "'";
							}#if
						}#if					
					}#for
    ...
    *fetchrow_arrayref = \&fetch; # required alias for fetchrow_arrayref
    ...
    sub DESTROY {
        my $sth = shift;
		my $dbh = $sth->FETCH('Database'); # get the dbh
        return unless $dbh->FETCH('Active');
		$sth->STORE('Active', 0);
		return undef;
    }
...
a 68K Mac are a bit out of luck, because I cannot provide a pre-build version of the Mac::DtfSQL extension 
module for them. One will need a Metrowerks Codewarrior compiler and linker to build a version for CFM 68K Macs 
(see the Mac::DtfSQL module documentation for more information).
...
Installation is quite easy. First, install the Mac build of the DBI 1.08 module, available from the MacPerl Module 
Porters Page at http://pudge.net/mmp/. As with every module, use Chris Nandor's B<installme.plx> droplet for 
installation. This installer is part of the cpan-mac-0.50 module, available from CPAN 
(http://www.perl.com/CPAN-local/authors/id/CNANDOR/) or via Chris Nandor's MacPerl page: http://pudge.net/macperl/.

After you've installed the DBI module, simply drop the packed archive C<DBD-DtfSQLmac-0.1201.tar.gz> or the
unpacked folder C<DBD-DtfSQLmac-0.1201> on the installme.plx droplet. Answer the upcoming question "Convert 
all text and MacBinary files?" with "Yes". This should install the module properly. To be sure, run the test.pl 
...
B<Important note:> This module can only be used in conjunction with the Mac::DtfSQL module, which is part of
...
shared library C<DtfSQL> built from the extension module (the folder is 
C<:site_perl:MacPPC:auto:Mac:DtfSQL:>) or put the dtF/SQL 2.01 shared library in the *system 
extensions* folder.
...
that comes with this module. The values should be regarded as the absolute minimum. As your database grows, be 
prepared to assign more memory to MacPerl. While the MacPerl application reports an "out of memory!" error, the 
MacPerl MPW tool reports a "Can't connect as user X" error. However, if you get an out of memory error, it's better 
to quit the corresponding application. If you try to run another script, you can crash your computer.
...
directory is the directory you are currently in with the MPW shell. You can use the C<`pwd`> command (as 
shown below) to figure out what the current working directory is:
 
    $curdir = `pwd`; # full path to the current working directory, followed by a newline
    chomp $curdir;
    $curdir =~ s/:$//; # get rid of trailing colon, if any (volume names only)
...
Work as expected. See the discussion on supported datatypes in the Statement Handle Attributes section.
...
B<Note> that the ? is *not* enclosed in quotation marks even when the placeholder represents a string. Every question
...
While the other defined DBI 1.08 SQL type numbers
...
will be mapped to their nearest corresponding DBI SQL type constant as shown, the three binary data types (blob) 

    SQL_BINARY
    SQL_VARBINARY
    SQL_LONGVARBINARY 

are *not supported* at all. You will get an error message if you specify one of these constants.
...
explicitely allows type *variants*, for example a signed or unsigned variant (see the documentation for the 
